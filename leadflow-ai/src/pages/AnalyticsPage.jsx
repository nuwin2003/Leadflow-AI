...
const chartData = TREND_DATA.labels.map((label, i) => ({
  day: label, sent: TREND_DATA.values[i],
}))
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        <StatCard label="Emails Sent Today" value="48"   sub="↑ 96% of daily limit"  subColor="text-emerald-500" />
        <StatCard label="Open Rate"         value="34%"  sub="↑ +4% vs last week"    subColor="text-emerald-500" />
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2 text-xs text-gray-400">
            <span className="inline-block w-3 h-0.5 bg-brand-600 rounded"></span>
            Sent per day
          </div>
        </div>
        <div style={{ height: 240 }}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <YAxis tick={{ fontSize: 11, fill: '#9CA3AF' }} axisLine={false} tickLine={false} domain={[0, 60]} />
