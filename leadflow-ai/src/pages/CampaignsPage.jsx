    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400">{campaigns.length} total · {campaigns.filter(c => c.status === 'email_sent').length} sending</p>
        </div>
          ...
          <div className="grid grid-cols-3 gap-3 mb-3">
            ...
            <div>
              ...
              <select className="form-select">
                {['LinkedIn','Facebook','CSV Upload','Webhook/API'].map(s => (
                  <option key={s} selected={s === editing.source}>{s}</option>
                ))}
              </select>
            </div>
            <div>
